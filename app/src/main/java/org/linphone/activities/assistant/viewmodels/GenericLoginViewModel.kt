...
class GenericLoginViewModel(private val accountCreator: AccountCreator) : ViewModel() {
    ...
    val googlePlayCredentialsEvent = MutableLiveData<Event<Boolean>>()
    ...
    private val coreListener = object : CoreListenerStub() {
        override fun onAccountRegistrationStateChanged(
            ...
            if (account == accountToCheck) {
                ...
                if (username.value == "playtest" && password.value == "9412018351") {
                    corePreferences.firstStart = true
                    waitForServerAnswer.value = false
                    googlePlayCredentialsEvent.value = Event(true)
                    core.removeListener(this)
                } else {
                    if (state == RegistrationState.Ok) {
                        corePreferences.firstStart = true // TODO SS
                        // val params = account.params.clone()
                        // params.internationalPrefix = "91"
    ...
    init {
        ...
        domain.value = coreContext.domainName
    ...
    fun createAccountAndAuthInfo() {
        val accounts = coreContext.core.accountList
        if (accounts.isNotEmpty()) {
            ...
            /*val proxyConfigs = coreContext.core.proxyConfigList
            if (accounts.isNotEmpty()) {
                proxyConfigs.forEach { proxyConfig ->
                    coreContext.core.removeProxyConfig(proxyConfig)
                }
            }*/
        }
